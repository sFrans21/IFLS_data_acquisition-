...
generate_shap_analysis.py
=========================
Analisis SHAP untuk model JUARA (champion) prediksi hipertensi.
(Champion dipilih dinamis di train_model.py; artifact = best_hipertensi_model.pkl.)
...
Cara pakai:
    1. Sesuaikan blok KONFIGURASI di bawah (path model & dataset).
    2. Letakkan best_hipertensi_model.pkl, train_hipertensi.csv, dan
       test_hipertensi.csv di folder yang sama.
    3. Jalankan:  python generate_shap_analysis.py
    4. Semua gambar + ringkasan angka (summary.json) tersimpan di ./shap_output/
"""
...
# ===================== KONFIGURASI =====================
MODEL_PATH   = "best_hipertensi_model.pkl"      # artifact hasil FASE 7 train_model.py
TRAIN_PATH   = "train_hipertensi.csv"              # keluaran data_preparation (sudah diimputasi)
TEST_PATH    = "test_hipertensi.csv"               # keluaran data_preparation (sudah diimputasi)
TARGET_COL   = "label_hypertension"                # kolom target (dibuang dari fitur)
OUTPUT_DIR   = "shap_output"                       # folder keluaran
...
def main():
    ...
    # ---------- 7. TULIS summary.json (PERBAIKAN D1) ----------
    # Sebelumnya dict 'summary' dibangun tetapi tidak pernah disimpan, sehingga
    # lapisan LLM (yang membaca summary.json sebagai sumber fakta) tidak punya
    # masukan. Di sini summary ditulis ke file agar rantai XAI -> LLM tersambung.
    out_json = os.path.join(OUTPUT_DIR, "summary.json")
    with open(out_json, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\nSelesai. summary.json + semua gambar tersimpan di: {OUTPUT_DIR}/")
    print("summary.json inilah yang dibaca lapisan LLM sebagai sumber fakta angka & arah.")
