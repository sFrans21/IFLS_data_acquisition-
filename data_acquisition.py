...
def main():
    ...
    # 1. Load data .dta (Stata)
    df_b3a = pd.read_stata("data/hh14_b3a_dta/b3a_cov.dta", columns=["hhid14", "pid14"])
    df_b3b = pd.read_stata("data/hh14_b3b_dta/b3b_cov.dta", columns=["hhid14", "pid14"])
    df_bus = pd.read_stata("data/hh14_bus_dta/bus_cov.dta", columns=["hhid14", "pid14"])

    # 2. Buat set pasangan (hhid14, pid14) dari data base sebagai acuan cepat
    keys_b3a = set(zip(df_b3a["hhid14"], df_b3a["pid14"]))


    # 3. Fungsi untuk memeriksa keberadaan data
    def cek_keberadaan(df_target, nama_data):
        # Gabungkan hhid14 dan pid14 menjadi pasangan tuple
        keys_target = set(zip(df_target["hhid14"], df_target["pid14"]))

        # Cari id yang TIDAK ada di data b3a
        id_hilang = keys_target - keys_b3a

        if len(id_hilang) == 0:
            print(
                f" Sukses: Semua individu di {nama_data} TERSEDIA di b3a_cov.dta"
            )
        else:
            print(
                f" Peringatan: Ada {len(id_hilang)} individu di {nama_data} yang TIDAK DITEMUKAN di b3a_cov.dta"
            )
            # Tampilkan beberapa contoh data yang hilang jika ada
            print("Contoh ID yang hilang (hhid14, pid14):", list(id_hilang)[:5])


    # 4. Jalankan pengecekan
    cek_keberadaan(df_b3b, "b3b_cov.dta")
    cek_keberadaan(df_bus, "bus_cov.dta")
    # 1. DEMOGRAFI
    print("\n[1/7] Memproses Demografi...")
    # df_ptrack, _ = load_data('hh14_trk_dta', 'ptrack.dta')
    df_cov, _ = load_data('hh14_b3a_dta', 'b3a_cov.dta')
    ...
    print("\n--- AKUISISI SELESAI (DATA MASIH MENTAH) ---")
    print(f"Dimensi Akhir: {master_df.shape}")
    print(f"Jumlah pidlink unik: {master_df[['hhid14', 'pid14']].drop_duplicates().shape[0]}")
