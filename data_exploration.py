...
"""
============================================================================
SCRIPT EKSPLORASI DATA AWAL (EDA) — VERSI HIPERTENSI
Skema kolom sesuai akuisisi terkini (data_acquisition.py):
  hhid14, pid14, sex, age, has_tobacco,
...
(Fitur diet FM2 tidak diakuisisi -> tidak ada di skema ini.)

Target  : hipertensi (diturunkan dari tekanan darah, JNC-7/WHO)
Catatan : kolom bmi TIDAK ada di dataset -> dihitung sendiri.
Keluaran: statistik di konsol + berkas PNG (g1..g8), satu grafik per berkas.
============================================================================
"""
...
#     # --- (1) Pembersihan kode tersamar -> NaN ---
    ...
    df["is_diabetes"]   = df["is_diabetes"].where(~df["is_diabetes"].isin([8, 9]))      # 8/9 -> NaN
    df["is_high_cholesterol"]   = df["is_high_cholesterol"].where(~df["is_high_cholesterol"].isin([8, 9]))      # 8/9 -> NaN
    df["is_kidney_disease"]   = df["is_kidney_disease"].where(~df["is_kidney_disease"].isin([8, 9]))      # 8/9 -> NaN  (F2: dulu terlewat)
    df["is_stroke"]   = df["is_stroke"].where(~df["is_stroke"].isin([8, 9]))      # 8/9 -> NaN
    df["moderate_act_last7d"]   = df["moderate_act_last7d"].where(~df["moderate_act_last7d"].isin([8, 9]))      # 8/9 -> NaN
    df["walking_last7d"]   = df["walking_last7d"].where(~df["walking_last7d"].isin([8, 9]))      # 8/9 -> NaN
    df["hard_act_last7d"]   = df["hard_act_last7d"].where(~df["hard_act_last7d"].isin([8, 9]))      # 8/9 -> NaN
...
#     --- (2) BMI di filter nilai outlier nya, baru dihitung sendiri ---
    ...
    # --- (3) Koreksi tekanan darah mustahil SEBELUM membuat label ---
    bad_bp = df["bp_diastolic"] >= df["bp_systolic"]   # F1: perbandingan NaN sudah otomatis False
    print(f"Tekanan darah mustahil disisihkan: {int(bad_bp.sum())} baris")
    df.loc[bad_bp, ["bp_systolic", "bp_diastolic"]] = np.nan
    ...
    # --- (5) Fitur biner untuk analisis bivariat (kode IFLS 1/3 -> 1/0) ---
    # F4: is_female tetap eksplisit; sisanya dipetakan dengan penjaga
    # 'if col in df.columns' agar EDA TIDAK crash bila suatu fitur (mis. diet)
    # tidak diakuisisi.
    df["is_female"] = df["sex"].map({1.0: 0, 3.0: 1})            # 3 = perempuan
    ...
    biner_ya1 = [
        "has_tobacco",
        "is_diabetes", "is_high_cholesterol", "is_kidney_disease", "is_stroke",
        "has_noodles", "has_fast_food", "has_fried_food",
        "hard_act_last7d", "moderate_act_last7d", "walking_last7d",
    ]
    for col in biner_ya1:
        if col in df.columns:
            df[col] = df[col].map({1.0: 1, 3.0: 0})   # 1 = ya, 3 = tidak
...
#     simpan("g8_ethnicity_hypertension.png")
    ...
    # F4: lewati plot diet bila kolomnya tidak diakuisisi (mencegah KeyError).
    feats = [(c, lbl) for c, lbl in feats if c in df.columns]
    if not feats:
        print("\n[INFO] Fitur diet tidak tersedia -> plot g8_biner_part3 dilewati.")
        return
